...
const isDirective = src =>
  src.startsWith(`k-`) || (src.startsWith('k') && src.length >= 2 && src[1] >= 'A' && src[1] <= 'Z')
...
/**
 * Parse kdu attribute from JSXAttribute
 * @param t
...
const parseAttributeJSXSpreadAttribute = (t, path, attributes, attributesArray) => {
  ...
  if (
    ...
    attributesArray.push({ type: 'kduSpread', argument: argument.node })
    attributes = {}
  }
  return attributes
}
...
const transformJSXElement = (t, path) => {
  ...
  if (attributes) {
    if (t.isArrayExpression(attributes)) {
      const helper = addDefault(path, '@kdujs/babel-helper-kdu-jsx-merge-props', { nameHint: '_mergeJSXProps' })
