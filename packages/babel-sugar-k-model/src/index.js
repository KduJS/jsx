...
/**
 * Parse kModel metadata
 *
 * @param  t
 * @param  path JSXAttribute
 * @returns null | Object<{ modifiers: Set<string>, valuePath: Path<Expression>}>
 */
const parseKModel = (t, path) => {
  if (t.isJSXNamespacedName(path.get('name')) || !startsWithCamel(path.get('name.name').node, 'k-model')) {
    return null
  }

  if (!t.isJSXExpressionContainer(path.get('value'))) {
    throw new Error('You have to use JSX Expression inside your k-model')
  }
...
const transformModel = (t, path, valuePath, modifiers) => {
  ...
  if (tag === 'select') {
    ...
    throw new Error(`kModel: ${tag}[type=${type}] is not supported`)
  }
